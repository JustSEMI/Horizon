...
if len(sys.argv) > 1 and sys.argv[1] == '--worker':
    ...
    # Shift arguments so the worker script sees the right sys.argv
    sys.argv = [sys.argv[0]] + sys.argv[3:]
    if worker_type == 'discord':
        runpy.run_module('main.discord_services', run_name='__main__')
    elif worker_type == 'scraper':
        runpy.run_module('main.scraper_service', run_name='__main__')
    sys.exit(0)
...
if getattr(sys, 'frozen', False):
    os.environ["DIR"] = os.path.dirname(sys.executable)
...
class DashboardApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        self.console_text.grid(row=1, column=0, padx=30, pady=(0, 30), sticky="nsew")
    ...
    def show_toast(self, message, duration=3000):
        ...
        toast = ctk.CTkFrame(self, fg_color=("#10b981", "#059669"), corner_radius=8)
    ...
    def start_scraper(self):
        ...
        self.scraper_thread = threading.Thread(target=self._read_scraper_logs, daemon=True)
        self.scraper_thread.start()

    def _read_scraper_logs(self):
